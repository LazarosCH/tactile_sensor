...
class MinimalSubscriber(Node):
    def __init__(self):
        ...
        self.subscription2 = self.create_subscription(Float32MultiArray, 'touch_propability_topic', self.listener_callback2, 10)         
        self.subscription1 = self.create_subscription(Float32MultiArray, 'force_array', self.listener_callback1, 10)
        ...
        self.Forces = np.zeros(len(self.x))
    ...
    def listener_callback3(self, msg):
        self.GT = min(np.array(msg.data)[0], self.maxvalue)
        self.new_gt = True
    ...
    def listener_callback1(self, msg):
        self.Forces = np.clip(np.array(msg.data),0,self.maxvalue)

        if self.propabilities[0]<0.5:
            mask=self.Forces>0.05
            self.sum= np.clip(np.sum(self.Forces[mask])*510-10, 0.0, self.maxvalue)
        else:
            self.sum=0.0
    ...
    def update_plot(self):
        # Only update if all subscribers have received new data
        if not (self.new_force and self.new_prop and self.new_gt):
            return

        self.t += 0.1   # because your loop is 0.1s
        ...
        for i, (xi, yi) in enumerate(self.datasets):
            ...
            alpha = float(np.clip(self.Forces[i], 0.0, 1.0))
            self.ax.fill(xi, yi, color='red', alpha=alpha)

        self.ax.text(-0.65, 3.2, f"F={self.sum:.1f}g", fontsize=16, color='black')
        ...
        self.ax2.plot(self.time_history, self.gt_history, '--', linewidth=2, label="Ground Truth")
        self.ax2.plot(self.time_history, self.force_history, '-o', linewidth=2, label="Predictions")      
        self.ax2.set_title("Force vs Time")
        self.ax2.set_xlabel("Time (s)")
        self.ax2.set_ylabel("Force (g)")
        self.ax2.grid(True)
        self.ax2.set_ylim(-10, 700)
        self.ax2.legend()
